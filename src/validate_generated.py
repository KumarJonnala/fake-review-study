...
ASPECT_PATTERNS = {
    "how convenient the location was": r"\b(?:locat|neighborhood|nearby|walk(?:ing)?\s+distance|block[s]?\s+from|downtown|close to|around the corner|steps from)",
    "whether it felt worth the price": r"\b(?:price[ds]?|pricing|cost|rate[s]?|value|expensive|cheap|afford|worth (?:it|the)|for what (?:we|i) paid|bargain|overpriced)",
    "how clean the room was": r"\b(?:clean|dirt(?:y|ier)|spotless|stain|dust|grime|hygien|immaculate|filth|housekeeping|tidy)",
    "whether the stay felt too short or too long": r"\b(?:\d+\s*(?:night|day|week)|one night|two nights|three nights|a week|weekend|stayed for|(?:our|the) stay|overnight|(?:another|extra|one more) (?:night|day))",
    "how the staff behaved": r"\b(?:staff|reception|front desk|concierge|manager|employee|housekeep|bellhop|porter|check[- ]?in)",
    "how quiet or noisy it was": r"\b(?:nois(?:e|y)|quiet|loud|sound(?:proof)?|hear(?:d|ing)?\s|thin walls|traffic outside|silent|peaceful)",
    "what breakfast was like": r"\b(?:breakfast|buffet|continental|morning meal|coffee (?:bar|station)|omelet|pastri)",
    "what the bathroom was like": r"\b(?:bathroom|shower|tub\b|toilet|sink|towel)",
    "how well you slept": r"\b(?:bed\b|beds\b|mattress|pillow|slept|sleep|duvet|linens)",
    "whether the decor felt dated or fresh": r"\b(?:decor|furnish|stylish|modern|dated|charming|tired|lobby|design)",
    "what the view was like": r"\b(?:view|overlook(?:ed|ing|s)?|out the window|window faced)",
    "whether the wifi worked": r"\b(?:wi[- ]?fi|internet|wireless|connectivity)",
}

# report_structure looks aspects up by exact string, so a rewording in config.py that is
# not mirrored here would score zero coverage SILENTLY. Fail at import instead.
_missing = set(cfg.ASPECTS) - set(ASPECT_PATTERNS)
if _missing:
    raise RuntimeError(
        f"cfg.ASPECTS entries have no ASPECT_PATTERNS regex: {sorted(_missing)}. "
        "Reword both in lockstep."
    )
