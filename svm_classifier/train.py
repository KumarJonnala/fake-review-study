...
def main():
    ...
    for i, params in enumerate(grid(grid_cfg), 1):
        model = SVC(probability=True, random_state=base["split"]["random_seed"], **params)
    ...
    out = Path(base["experiment"]["output_dir"]) / "svm"
    out.mkdir(parents=True, exist_ok=True)
    save_json({"validation_results": results}, out / "validation_results.json")

    best = results[0]["params"]
    final = SVC(probability=True, random_state=base["split"]["random_seed"], **best)
